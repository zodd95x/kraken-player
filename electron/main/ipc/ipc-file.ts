...
import { getMainText } from "../utils/localization";
...
const initFileIpc = (): void => {
  ...
  // 默认下载目录
  ipcMain.handle("get-default-download-dir", (): string => {
    return join(app.getPath("music"), "Kraken Player");
  });
  ...
  ipcMain.handle("get-downloaded-songs", async (_event, dirPath: string) => {
    try {
      ...
      try {
        const coverDir = getCoverDir();
        // 尝试通过本地音乐服务扫描
        const scanned = await localMusicService.scanDirectory(dirPath);
        if (scanned && scanned.length > 0) {
          tracks = processMusicList(scanned, coverDir);
        }
      } catch (scanErr) {
        ipcLog.warn("LocalMusicService scan error, falling back to MusicMetadataService:", scanErr);
      }

      // 兜底策略：当原生模块不可用或结果为空时，通过 Node.js 元数据服务直接读取目录
      if (!tracks || tracks.length === 0) {
        tracks = await musicMetadataService.scanDirectory(dirPath);
      } else {
        // 原生扫描不读下载索引：用索引 + 同名图回填缺失封面与信息
        const isUnknownText = (v: unknown): boolean => {
          if (!v) return true;
          if (Array.isArray(v)) return v.length === 0;
          return (
            typeof v === "string" &&
            ["Unknown Artist", "Artiste inconnu", "未知艺术家", "unknown"].includes(v.trim())
          );
        };
        const metaKeysOf = (p: string): string[] => {
          const base = p.replace(/\\/g, "/").split("/").pop() || p;
          const noExt = base.replace(/\.[^.]+$/, "");
          return [base, noExt, `${noExt}${p.match(/\.[^.]+$/)?.[0] || ""}`, p];
        };
        try {
          const metaMap = await musicMetadataService.readDownloadMetaIndex(dirPath);
          await Promise.all(
            tracks.map(async (track) => {
              const keys = track.path ? metaKeysOf(track.path) : [];
              const meta = keys.map((k) => metaMap[k]).find((m) => m !== undefined && m !== null);
              if (!track.cover && track.path) {
                const cover = await musicMetadataService.resolveDownloadCover(track.path, metaMap);
                if (cover) track.cover = cover;
              }
              // 索引里有准确的下载时信息，缺失才回填
              if (meta) {
                if (meta.name && (!track.name || keys.includes(track.name))) {
                ...
                const artist =
                  ...
                  (Array.isArray(meta.artists)
                    ...
                    : meta.artists);
                if (artist && isUnknownText(track.artists)) {
                  track.artists = artist;
                  track.artist = artist;
                }
                if (
                  meta.album &&
                  (!track.album || ["Unknown Album", "未知专辑"].includes(track.album))
                ) {
  ...
  ipcMain.handle("choose-path", async (_, title: string, multiSelect: boolean = false) => {
    try {
      ...
      const { filePaths } = await dialog.showOpenDialog({
        title: title ?? getMainText("chooseFolder"),
        defaultPath: app.getPath("downloads"),
        properties,
        buttonLabel: getMainText("chooseFolder"),
      });
      if (!filePaths || filePaths.length === 0) return null;
      // 多选时返回数组，单选时返回第一个路径

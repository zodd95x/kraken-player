...
impl TaskbarStrategy for LegacyStrategy {
    ...
    fn embed_window(&self, child_wnd: HWND) -> bool {
        self.h_child.set(child_wnd.0 as usize);
    ...
    fn update_layout(&mut self, params: LayoutParams) -> Option<TaskbarLayout> {
        ...
        unsafe {
            ...
            // 自愈：父窗口丢失时重新嵌入，再顶到最上
            let child = HWND(self.h_child.get() as *mut c_void);
            super::ensure_embedded(child, self.h_taskbar);
            // 布局变化时任务栏可能重排兄弟窗口，每次都重新顶到最上
            super::raise_child_window(child);

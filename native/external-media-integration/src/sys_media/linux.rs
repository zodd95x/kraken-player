...
async fn process_metadata_update(
    ...
    let track_path = format!("/com/krakenplayer/track/{track_id_str}");
...
async fn run_mpris_loop(mut rx: UnboundedReceiver<MprisCommand>) -> Result<()> {
    ...
    let pid = process::id();
    // 使用唯一标识符以避免多个实例冲突
    let identity = format!("kraken-player.instance{pid}");
    ...
    let player = Player::builder(&identity)
        ...
        .identity("Kraken Player")
        .desktop_entry("com.krakenplayer.app")
        .build()
        .await

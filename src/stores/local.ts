...
const createLocalStore = () => {
  ...
  // 歌单歌曲完整信息缓存
  const playlistSongsCache = ref<Record<string, SongType>>({});
  // 是否初始化完成
  const isInitialized = ref(false);

  // 读取歌单歌曲缓存
  const readPlaylistSongsCache = async () => {
    try {
      const result = await localDB.getItem("playlist-songs-cache");
      playlistSongsCache.value = (result as Record<string, SongType>) || {};
    } catch (error) {
      console.error("Error reading playlist songs cache:", error);
    }
  };
  ...
  const updatePlaylistCover = async (
    ...
    const firstSong =
      localSongs.value.find((s) => s.id.toString() === firstSongId) ||
      playlistSongsCache.value[firstSongId];
    if (firstSong?.cover) {
  ...
  const addSongsToLocalPlaylist = async (
    ...
    if (songObjects && songObjects.length > 0) {
      songObjects.forEach((song) => {
        playlistSongsCache.value[song.id.toString()] = song;
      });
      await localDB.setItem("playlist-songs-cache", cloneDeep(playlistSongsCache.value));
    }
  ...
  const getLocalPlaylistDetail = (
    ...
    const songs = playlist.songs
      .map((songId) => songsMap.get(songId) || playlistSongsCache.value[songId])

    ...
    >
      <div class="header" align="center" justify="space-between">
        ...
        <n-flex :wrap="false" align="center" justify="center" size="small" @pointerdown.stop>
          ...
          <div
            class="menu-btn"
            :title="trSetting(lyricData.playStatus ? '暂停' : '播放')"
...
const renderLyricLines = computed<RenderLine[]>(() => {
  ...
  if (!lyricData.playName && !lyrics?.length) {
    return placeholder("Kraken Player Desktop Lyric");
  }
  // 加载中
  if (lyricData.lyricLoading && !lyrics?.length) return placeholder(trSetting("歌词加载中..."));
  // 纯音乐
  if (!lyrics?.length) return placeholder(trSetting("纯音乐，请欣赏"));
  // 获取当前歌词索引

  ...
  <div class="streaming">
      ...
      <div :key="pageTitle" class="title">
        ...
        <n-flex v-if="isConnected" class="status">
          <n-text class="item">
            ...
            <n-number-animation :from="0" :to="songsCount" /> titre{{ songsCount > 1 ? "s" : "" }}
          </n-text>
          <n-text class="item server-info">
            <SvgIcon name="Stream" :depth="3" />
            {{ streamingStore.activeServer.value?.type || "Serveur inconnu" }}
          </n-text>
        </n-flex>
      </div>
    </Transition>
    <n-flex class="menu" justify="space-between">
      ...
      <n-flex class="right" justify="end">
        ...
        >
          <n-tab :disabled="tabsDisabled" name="streaming-songs"> {{ trSetting("单曲") }} </n-tab>
          <n-tab :disabled="tabsDisabled" name="streaming-artists"> {{ trSetting("歌手") }} </n-tab>
          <n-tab :disabled="tabsDisabled" name="streaming-albums"> {{ trSetting("专辑") }} </n-tab>
          <n-tab :disabled="tabsDisabled" name="streaming-playlists">
            {{ trSetting("歌单") }}
          </n-tab>
        </n-tabs>
      </n-flex>
...
// 页面标题
const pageTitle = computed<string>(() => "Streaming");
...
const emptyDescription = computed<string>(() => {
  if (!streamingStore.hasServer.value) {
    return trSetting("未配置流媒体服务器");
  }
  if (!isConnected.value) {
    return trSetting("流媒体服务器未连接");
  }
  return trSetting("暂无歌曲");
});
...
const tabLabels = computed<Record<string, string>>(() => ({
  "streaming-songs": trSetting("单曲"),
  "streaming-artists": trSetting("歌手"),
  "streaming-albums": trSetting("专辑"),
  "streaming-playlists": trSetting("歌单"),
}));

// Tab 下拉选项
const tabDropdownOptions = computed<DropdownOption[]>(() => [
  { label: trSetting("单曲"), key: "streaming-songs", icon: renderIcon("Music") },
  { label: trSetting("歌手"), key: "streaming-artists", icon: renderIcon("Artist") },
  { label: trSetting("专辑"), key: "streaming-albums", icon: renderIcon("Album") },
  { label: trSetting("歌单"), key: "streaming-playlists", icon: renderIcon("MusicList") },
]);

// 当前 Tab 标签
const currentTabLabel = computed(() => tabLabels.value[streamingType.value] || trSetting("单曲"));
...
const handleServerChange = async (serverId: string) => {
  ...
  try {
    ...
    if (success) {
      window.$message.success(trSetting("服务器切换成功"));
      await loadData();
    } else {
      window.$message.error(
        trSetting(streamingStore.connectionStatus.value.error || "切换服务器失败"),
      );
    }
  } catch (error) {
    window.$message.error(
      trSetting("切换失败：") + (error instanceof Error ? error.message : trSetting("未知错误")),
    );
  } finally {
    loading.value = false;
  }
...
const openServerConfig = () => {
  ...
  openStreamingServerConfig(editingServer, async (config) => {
    try {
      ...
      const success = await streamingStore.connectToServer(serverId);
      if (success) {
        window.$message.success(trSetting("连接成功"));
        await loadData();
      } else {
        window.$message.error(
          streamingStore.connectionStatus.value.error
            ? trSetting(streamingStore.connectionStatus.value.error)
            : trSetting("连接失败"),
        );
      }
    } catch (error) {
      window.$message.error(
        `${trSetting("连接失败")} : ` +
          (error instanceof Error ? error.message : trSetting("未知错误")),
      );
    }
  });
};
...
const handleDisconnect = () => {
  window.$dialog.warning({
    title: "Déconnexion",
    content: "Êtes-vous sûr de vouloir vous déconnecter du serveur de streaming ?",
    positiveText: "Confirmer",
    negativeText: "Annuler",
    onPositiveClick: () => {
      streamingStore.disconnect();
      window.$message.info("Déconnecté");
    },
  });
};
...
const loadData = async () => {
  ...
  try {
    ...
    window.$message.error(trSetting("加载流媒体数据失败"));
  } finally {
    loading.value = false;
  }
};

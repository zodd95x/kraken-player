<template>
  <div class="streaming-artists">
    <n-scrollbar class="artist-list">
      ...
      >
        <n-text class="name">{{ key || "Artiste inconnu" }}</n-text>
        <n-text class="num" depth="3">
          <SvgIcon name="Music" :depth="3" />
          {{ item.length }} titre{{ item.length > 1 ? "s" : "" }}
        </n-text>
      </n-card>
    </n-scrollbar>
...
const formatArtistsList = (data: SongType[]): Record<string, SongType[]> => {
  ...
  const allArtists = data.reduce(
    (acc, song) => {
      // 歌手信息
      const artistName = typeof song.artists === "string" ? song.artists.trim() : "Artiste inconnu";
      // 若还无歌手分类，初始化为空数组

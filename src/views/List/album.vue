    ...
    >
      <template #action-buttons>
        <n-button
          ...
          secondary
          round
          @click="toLikeAlbum(albumId, !isLikeAlbum, detailData || undefined)"
        >
          <template #icon>
            <SvgIcon :name="isLikeAlbum ? 'Favorite' : 'FavoriteBorder'" />
          </template>
          {{ isLikeAlbum ? trSetting("取消收藏专辑") : trSetting("收藏专辑") }}
        </n-button>
      </template>
    </ListDetail>
    <SongList
...
const moreOptions = computed<DropdownOption[]>(() => [
  ...
  },
  {
    ...
    props: {
      onClick: () =>
        copyData(getShareUrl("album", albumId.value), trSetting("分享链接已复制到剪贴板")),
    },

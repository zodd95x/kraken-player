    ...
    >
      <template #action-buttons>
        ...
        <n-button
          ...
          secondary
          round
          @click="toLikePlaylist(playlistId, !isLikePlaylist, detailData || undefined)"
        >
          <template #icon>
            <SvgIcon :name="isLikePlaylist ? 'Favorite' : 'FavoriteBorder'" />
          </template>
          {{ isLikePlaylist ? trSetting("取消收藏") : trSetting("收藏歌单") }}
        </n-button>
      </template>
    </ListDetail>
    <!-- 歌曲列表 -->
...
const moreOptions = computed<DropdownOption[]>(() => [
  ...
  },
  {
    ...
    props: {
      onClick: () =>
        copyData(getShareUrl("playlist", playlistId.value), trSetting("分享链接已复制到剪贴板")),
    },
...
const getPlaylistDetail = async (
  ...
  if (id.toString().length === 16 && !localStore.isInitialized) {
    try {
      ...
      window.$message.error(trSetting("无法加载本地歌单"));
  ...
  // 在线歌单
    ...
    try {
      ...
      window.$message.error(trSetting("无法加载歌单详情"));
      setLoading(false);
      router.push("/");
    }
  }
...
const handleLocalPlaylist = (id: number) => {
  ...
  if (!result) {
    window.$message.error(trSetting("本地歌单不存在"));
    setLoading(false);
    return;
  }
...
const loadingMsgShow = (show: boolean = true, count?: number) => {
  if (show) {
    ...
    loadingMsg.value = window.$message.loading(trSetting("该歌单歌曲较多，请稍候..."), {
      duration: 0,
      closable: true,
    });
  } else {
...
const toDeletePlaylist = async () => {
  ...
  window.$dialog.warning({
    title: trSetting("删除歌单"),
    content: trSetting("确定删除歌单？该操作不可逆！"),
    ...
    onPositiveClick: async () => {
      ...
      if (isLocalPlaylist.value) {
        ...
        if (success) {
          window.$message.success(trSetting("本地歌单删除成功"));
          router.back();
        } else {
      ...
      if (result.code === 200) {
        window.$message.success(trSetting("歌单删除成功"));
        // 更新用户歌单
...
const handleReorder = async (fromIndex: number, toIndex: number) => {
  ...
  if (isLocalPlaylist.value) {
    ...
    if (!success) {
      window.$message.error(trSetting("排序调整失败"));
    ...
    try {
      ...
      if (result.code !== 200) {
        window.$message.error(trSetting("保存排序失败"));
      ...
      window.$message.error(trSetting("保存排序失败，请重试"));
      getPlaylistDetail(playlistId.value, { getList: true, refresh: true });
    }
  }
};
...
const openPrivacy = async () => {
  ...
  window.$dialog.warning({
    title: trSetting("公开歌单"),
    content: trSetting("确定公开歌单？该操作不可逆！"),
    positiveText: trSetting("设为公开"),
    negativeText: trSetting("取消"),
    onPositiveClick: async () => {
      const result = await updatePlaylistPrivacy(playlistId.value);
      if (result.code !== 200) return;
      if (detailData.value) detailData.value.privacy = 0;
      window.$message.success(trSetting("歌单已公开"));
    },
  });
};

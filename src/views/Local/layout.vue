<template>
  <div class="local">
      ...
      <div :key="pageTitle" class="title">
        ...
        <n-flex class="status">
          <n-text class="item">
            ...
            <n-number-animation :from="0" :to="listData?.length || 0" /> {{ trSetting("首歌曲") }}
          </n-text>
          <n-text class="item">
    ...
    <n-flex v-else align="center" justify="center" vertical class="router-view">
      <n-empty size="large" :description="trSetting('暂无本地音乐')">
        <template #extra>
...
const folderOptions = computed(() => {
  const options: { label: string; value: string }[] = [
    { label: trSetting("所有文件夹"), value: "all" },
  ];
...
const pageTitle = computed<string>(() => {
  if (settingStore.useOnlineService) return trSetting("本地音乐");
  // 本地模式
  const routeName = router.currentRoute.value?.name as string;
  switch (routeName) {
    case "local-songs":
    case "local":
      return trSetting("音乐库");
...
const getAllLocalMusic = debounce(
  async (showTip: boolean = false) => {
    ...
    if (!allPath || !allPath.length) {
      ...
      loading.value = false;
      if (showTip) {
        window.$message.info(trSetting("未配置本地文件夹"));
      }
      return;
    }
    ...
    // 加载提示
    if (showTip) {
      loadingMsg.value = window.$message.loading(trSetting("正在同步本地歌曲..."), {
        duration: 0,
      });
    ...
    const completeHandler = (_event: unknown, data: SyncCompleteData) => {
      ...
      isCompleted = true;
      if (!data.success) {
        const errorMsg = trSetting(data.message || "本地同步失败");
        ...
        if (addedCount > 0) {
          window.$message.success(
            settingStore.language === "en"
              ? `${addedCount} new track(s) added`
              : settingStore.language === "zh"
                ? `已添加 ${addedCount} 首新歌曲`
                : `${addedCount} nouveau(x) titre(s) ajouté(s)`,
          );
        } else if (finalSongs.length > 0) {
          window.$message.success(
            settingStore.language === "en"
              ? `${finalSongs.length} track(s) found`
              : settingStore.language === "zh"
                ? `找到 ${finalSongs.length} 首歌曲`
                : `${finalSongs.length} titre(s) trouvé(s)`,
          );
        }
      } else {
        if (addedCount > 0) {
    ...
    try {
      ...
      if (res && !res.success) {
        ...
        if (res.message && res.message.includes("扫描正在进行中")) {
          window.$message.info(trSetting(res.message));
        } else {
          window.$message.error(trSetting(res.message || "本地同步失败"));
        }
        window.electron.ipcRenderer.removeAllListeners("music-sync-tracks-batch");
        window.electron.ipcRenderer.removeAllListeners("music-sync-complete");
      }
      ...
      window.$message.error(trSetting("获取本地音乐失败，请重试"));
      loading.value = false;
      loadingMsg.value?.destroy();
      loadingMsg.value = null;
      // 清理监听器
...
onMounted(() => {
  ...
  const progressHandler = (_event: unknown, payload: { current: number; total: number }) => {
    ...
    if (loadingMsg.value) {
      loadingMsg.value.content = `Analyse des fichiers locaux (${current}/${total})...`;
    }
  };
  // 监听进度

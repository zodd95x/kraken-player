<template>
  <div class="cloud">
    <div class="title">
      <n-text class="keyword">Cloud Personnel</n-text>
      <n-flex class="status">
        <n-text class="item">
          <SvgIcon name="Music" :depth="3" />
          <n-number-animation :from="0" :to="cloudCount || cloudData?.length || 0" /> titre(s)
        </n-text>
        <n-text class="item">
    ...
    <n-flex class="menu" justify="space-between">
      <n-flex class="left" align="flex-end">
        ...
        >
          ...
          {{
            showLoading
              ? `${trSetting("加载中...")} (${cloudData.length}/${cloudCount})`
              : trSetting("播放全部")
          }}
        </n-button>
    ...
    <Transition name="fade" mode="out-in">
      ...
      <n-empty
        v-else
        :description="`Désolé, aucun titre trouvé pour ${searchValue}`"

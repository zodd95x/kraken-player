      ...
      <div v-if="videoData" class="info">
        <n-h2 class="name">
          <n-ellipsis :line-clamp="1" :tooltip="{ placement: 'bottom' }">
            {{ videoData.name || trSetting("Vidéo inconnue") }}
          </n-ellipsis>
        </n-h2>
      ...
      <n-flex :key="videoData?.id" class="menu" justify="space-between" align="center">
        ...
        >
          <n-avatar
            ...
            round
          />
          <n-text class="name">{{ artistData?.name || trSetting("Artiste inconnu") }}</n-text>
        </div>
    ...
    <Transition name="fade" mode="out-in">
      <div v-if="videoData" class="desc">
        ...
        <n-ellipsis :line-clamp="3" :tooltip="{ placement: 'bottom', width: 'trigger' }">
          {{ videoData?.description || trSetting("Aucune description pour cette vidéo") }}
        </n-ellipsis>
...
const playerOptions: Plyr.Options = {
  ...
  i18n: {
    ...
    pause: trSetting("暂停"),
    speed: trSetting("速度"),
    settings: trSetting("设置"),
    normal: trSetting("正常"),
    quality: trSetting("音质"),
    pip: trSetting("画中画"),
    enterFullscreen: trSetting("进入全屏"),
    exitFullscreen: trSetting("退出全屏"),
    mute: trSetting("静音"),
    unmute: trSetting("取消静音"),
  },
  tooltips: {
    controls: true,
  },
...
const getVideoData = async (id: number, type: "mv" | "video") => {
  try {
    ...
    if (isEmpty(brs)) {
      window.$message.error(trSetting("获取视频播放地址失败"));
      return;
    }
    ...
    window.$message.error(trSetting("获取视频详情失败"));
  }
};

<template>
  <div class="discover">
    <div class="title">
      <n-text class="keyword">{{ trSetting("发现音乐") }}</n-text>
    </div>
    <!-- 标签页 -->
    <n-tabs
    ...
    >
      <n-tab name="discover-playlists"> {{ trSetting("歌单广场") }} </n-tab>
      <n-tab name="discover-toplists"> {{ trSetting("排行榜") }} </n-tab>
      <n-tab name="discover-artists"> {{ trSetting("歌手") }} </n-tab>
      <n-tab name="discover-new"> {{ trSetting("最新音乐") }} </n-tab>
    </n-tabs>
    <!-- 路由 -->

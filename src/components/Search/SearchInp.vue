...
// 搜索框默认提示
const defaultSearchText = computed(() => {
  if (settingStore.language === "zh") {
    return settingStore.useOnlineService ? "搜索音乐 / 视频" : "搜索本地音乐";
  }
  ...
  return settingStore.useOnlineService
    ? "Rechercher des titres, artistes, albums..."
    : "Rechercher dans la musique locale...";
});

// 搜索框数据
const searchInputRef = ref<HTMLInputElement | null>(null);
const searchPlaceholder = ref<string>(defaultSearchText.value);
...
const updatePlaceholder = async () => {
  if (!settingStore.enableSearchKeyword || settingStore.language !== "zh") {
    searchPlaceholder.value = defaultSearchText.value;
    return;
  }
  try {

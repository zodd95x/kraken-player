<template>
  <div class="copy-lyrics">
    ...
    <n-flex vertical size="small" class="footer">
      <n-text depth="2" class="footer-title">Éléments à copier</n-text>
      <n-checkbox-group v-model:value="selectedFilters">
        <n-flex align="center" wrap :size="12" class="footer-options">
          <n-checkbox value="translation" label="Traduction" />
          <n-checkbox value="romaji" label="Phonétique (Romaji)" />
          <n-checkbox
            value="emptyLine"
            label="Ligne vide"
            title="Insérer une ligne vide entre chaque ligne"
          />
          <n-checkbox value="songName" label="Titre" />
          <n-checkbox value="artist" label="Artiste" />
        </n-flex>
      </n-checkbox-group>
...
const handleCopy = async () => {
  ...
  if (linesToCopy) {
    ...
    window.$message.warning("Aucun contenu à copier");
  }
};
</script>

<template>
  <div class="song-info-editor">
    <n-tabs type="segment" animated>
      <n-tab-pane name="info" :tab="trSetting('信息')" display-directive="show">
        <!-- 在线匹配 -->
        <n-flex class="match" justify="space-between" align="center">
          <n-text>{{ trSetting("在线匹配歌曲信息？") }}</n-text>
        ...
        <n-scrollbar class="scrollbar">
          <n-form ref="infoFormRef" :model="infoFormData" :rules="infoFormRules" class="phone-form">
            <n-form-item :label="trSetting('文件名')" path="fileName">
              <n-input v-model:value="infoFormData.fileName" disabled />
            </n-form-item>
            <n-form-item :label="trSetting('歌曲名')" path="name">
              <n-input
                v-model:value="infoFormData.name"
                :placeholder="trSetting('请输入歌曲名')"
                clearable
              />
            </n-form-item>
            <n-form-item :label="trSetting('歌手')" path="artist">
              <n-input
                v-model:value="infoFormData.artist"
                :placeholder="trSetting('请输入歌手名')"
                clearable
              />
            </n-form-item>
            <n-form-item :label="trSetting('专辑')" path="album">
              <n-input
                v-model:value="infoFormData.album"
                :placeholder="trSetting('请输入专辑名')"
                clearable
              />
            </n-form-item>
            <n-form-item :label="trSetting('专辑歌手')" path="albumArtist">
              <n-input
                v-model:value="infoFormData.albumArtist"
                :placeholder="trSetting('请输入专辑歌手名')"
                clearable
              />
            </n-form-item>
            <n-form-item :label="trSetting('流派')" path="genre">
              <n-input
                v-model:value="infoFormData.genre"
                :placeholder="trSetting('请输入音乐流派')"
                clearable
              />
            </n-form-item>
            <n-grid :cols="24" :x-gap="24">
              <n-form-item-gi :span="8" :label="trSetting('年份')" path="year">
                <n-input-number
                  v-model:value="infoFormData.year"
                  :show-button="false"
                  :placeholder="trSetting('年份')"
                  style="width: 100%"
                  clearable
                />
              </n-form-item-gi>
              <n-form-item-gi :span="8" :label="trSetting('音轨号')" path="trackNumber">
                <n-input-number
                  v-model:value="infoFormData.trackNumber"
                  :show-button="false"
                  :placeholder="trSetting('音轨')"
                  style="width: 100%"
                  clearable
                />
              </n-form-item-gi>
              <n-form-item-gi :span="8" :label="trSetting('光盘号')" path="discNumber">
                <n-input-number
                  v-model:value="infoFormData.discNumber"
                  :show-button="false"
                  :placeholder="trSetting('光盘')"
                  style="width: 100%"
                  clearable
                />
              </n-form-item-gi>
            </n-grid>
            <n-form-item :label="trSetting('别名')" path="alia">
              <n-input
                v-model:value="infoFormData.alia"
                :placeholder="trSetting('请输入别名')"
                clearable
              />
            </n-form-item>
            <n-form-item :label="trSetting('歌词')" path="lyric">
              <n-input
                v-model:value="infoFormData.lyric"
                :autosize="{ minRows: 3, maxRows: 6 }"
                :placeholder="trSetting('请输入或粘贴歌词')"
                type="textarea"
              />
            </n-form-item>
            <n-grid :cols="24" :x-gap="24">
              <n-form-item-gi :span="12" :label="trSetting('格式')" path="type">
                <n-input v-model:value="infoFormData.type" disabled />
              </n-form-item-gi>
              <n-form-item-gi :span="12" :label="trSetting('比特率')" path="br">
                <n-input-number
              ...
              <n-form-item-gi :span="12" :label="trSetting('时长')" path="duration">
                <n-input-number
              ...
              <n-form-item-gi :span="12" :label="trSetting('采样率')" path="br">
                <n-input-number
              ...
              <n-form-item-gi :span="12" :label="trSetting('路径')" path="path">
                <n-input-group>
      ...
      <n-tab-pane name="cover" :tab="trSetting('封面')" display-directive="show">
        <n-image
          :src="coverData"
        ...
        <n-flex class="menu" justify="center">
          <n-text depth="3">{{ trSetting("点击封面即可修改") }}</n-text>
        </n-flex>
      </n-tab-pane>
    </n-tabs>
    <n-flex class="menu" justify="center">
...
const onlineMatch = debounce(
  async () => {
    try {
      ...
      if (isEmpty(song)) {
        window.$message.error("Aucune correspondance trouvée, veuillez modifier les informations");
        return;
      } else {
        ...
        window.$message.success("Correspondance trouvée avec succès");
      }
    } catch (error) {
      console.error("Error online matching:", error);
      window.$message.error("Erreur lors de la recherche de correspondance");
    }
  },
  300,
...
const saveSongInfo = debounce(async (song: SongType) => {
  try {
    ...
    window.$message.success("Informations du titre enregistrées");
    // 修改音乐信息
    updatePlaySong(metadata);
    emit("close");
  } catch (error) {
    console.error("Error saving song info:", error);
    window.$message.error("Échec de l'enregistrement, veuillez réessayer");
  }
}, 300);

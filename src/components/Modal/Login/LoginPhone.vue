<template>
  <div class="login-phone">
    <n-form ref="phoneFormRef" :model="phoneFormData" :rules="phoneFormRules" class="phone-form">
      <n-form-item :label="trSetting('国家')" path="country">
        <n-select v-model:value="phoneFormData.country" filterable :options="countryListData" />
      </n-form-item>
      <n-form-item :label="trSetting('手机号')" path="phone">
        <n-input-number
          v-model:value="phoneFormData.phone"
          :show-button="false"
          :placeholder="trSetting('请输入手机号')"
          passively-activated
          clearable
        >
          <template #prefix>
      ...
      <n-form-item :label="trSetting('验证码')" path="captcha">
        <n-input-number
          v-model:value="phoneFormData.captcha"
          :show-button="false"
          :disabled="phoneFormData.phone === null"
          :placeholder="trSetting('请输入短信验证码')"
          passively-activated
          clearable
        >
          <template #prefix>
      ...
      <n-form-item :show-label="false">
        <n-button class="login" type="primary" @click="login"> {{ trSetting("登录") }} </n-button>
      </n-form-item>
    </n-form>
  </div>
</template>
...
const phoneFormRules = computed<FormRules>(() => ({
  country: { ...numberRule, message: trSetting("请选择国家") },
  phone:
    phoneFormData.value.country === 86
      ...
      : {
          ...numberRule,
          key: "phone",
          message: trSetting("请输入手机号"),
        },
  captcha: { ...numberRule, message: trSetting("请输入正确的验证码") },
}));

// 验证码数据
const captchaTime = ref<number>(60);
const captchaText = ref<string>(trSetting("获取验证码"));
...
const { pause: pauseTime, resume: resumeTime } = useIntervalFn(
  () => {
    ...
    if (captchaTime.value <= 0) {
      ...
      captchaText.value = trSetting("重新获取");
      captchaDisabled.value = false;
    }
  },
  1000,

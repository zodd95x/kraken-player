<template>
  <div class="login-uid">
    <n-alert :bordered="false" :title="trSetting('如何获取 UID')">
      <template #icon>
        <SvgIcon name="Help" />
      </template>
      {{ trSetting("可前往") }}
      <n-a href="https://music.163.com/" target="_blank">{{ trSetting("网易云音乐") }}</n-a>
      {{
        trSetting(
    ...
    <n-input-number
      v-model:value="uid"
      :show-button="false"
      :placeholder="trSetting('请输入 UID')"
    />
    <n-button :loading="!!loadingMsg" type="primary" @click="login">{{
      trSetting("登录")
    }}</n-button>
  </div>
</template>

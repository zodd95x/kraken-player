<template>
  <n-scrollbar style="max-height: 70vh" class="font-manager">
    <div class="set-list">
      <n-h3 prefix="bar">Police générale</n-h3>
      <n-card v-if="isElectron" class="set-item">
        <div class="label">
          <n-text class="name">Style de configuration de police</n-text>
        ...
        <n-select
          ...
          :options="[
            {
              label: 'Police CSS personnalisée',
              value: 'custom',
            },
            {
        ...
        <div class="label">
          <div style="display: flex; justify-content: space-between; align-items: center">
            <div class="info" style="display: flex; flex-direction: column">
              <n-text class="name">Police globale</n-text>
              <n-text class="tip" :depth="3"
                >Police appliquée à l'ensemble de l'interface logicielle</n-text
              >
            </div>
    ...
    <div class="set-list" v-if="isElectron">
      <n-h3 prefix="bar">Paroles sur le bureau</n-h3>
      <n-card class="set-item" :class="{ 'input-mode': isInputMode }">
        <div class="label">
          <div class="label-header">
            <div class="info" style="display: flex; flex-direction: column">
              <n-text class="name">Police des paroles sur le bureau</n-text>
              <n-text class="tip" :depth="3"> Police utilisée pour les paroles flottantes </n-text>
            </div>
            <Transition name="fade" mode="out-in">
              <n-button
      ...
      <n-card class="set-item" :class="{ 'input-mode': isInputMode }">
        <div class="label">
          <div class="label-header">
            <div class="info" style="display: flex; flex-direction: column">
              <n-text class="name">Police des paroles de la barre des tâches</n-text>
              <n-text class="tip" :depth="3">
                Police utilisée pour les paroles dans la barre des tâches
              </n-text>
            </div>
            <Transition name="fade" mode="out-in">
              <n-button
    ...
    <div class="set-list">
      <n-h3 prefix="bar">Police des paroles</n-h3>
      <n-card
...
const getOptions = (key: string) => {
  ...
  let defaultLabel = "Suivre la police globale";
  let defaultValue = "follow";

  if (isGlobal || isDesktop) {
    defaultLabel = "Police système par défaut";
...
const saveDesktopLyricConfig = (val?: string) => {
  try {
    ...
    window.$message.success("Police des paroles de bureau enregistrée");
  } catch (error) {
    console.error("Failed to save options:", error);
    window.$message.error("Échec de l'enregistrement de la configuration");

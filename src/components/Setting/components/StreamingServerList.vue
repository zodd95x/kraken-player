<template>
  <n-card class="set-item" id="server-list-choose" content-style="flex-direction: column">
    <n-flex justify="space-between">
      <div class="label">
        <n-text class="name">{{ trSetting(item?.label) || trSetting("服务器列表") }}</n-text>
        <n-text
          class="tip"
          :depth="3"
    ...
    <n-collapse-transition :show="servers.length > 0">
      ...
      >
        ...
        <n-flex class="set" justify="end" :size="8">
          ...
          <!-- 删除 -->
          <n-popconfirm
            :positive-text="trSetting('删除')"
            :negative-text="trSetting('取消')"
          ...
          >
            ...
            {{ trSetting("确定要删除服务器") }} "{{ server.name }}" ？
          </n-popconfirm>
        </n-flex>
      </n-card>

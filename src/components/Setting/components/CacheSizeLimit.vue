<template>
  <n-card class="set-item">
    <div class="label">
      <n-text class="name">{{ trSetting(item?.label) || trSetting("缓存大小上限") }}</n-text>
      <n-text
        class="tip"
        :depth="3"
      ...
      <n-text class="tip" :depth="3" v-else>
        {{ trSetting("达到上限后将清理最旧的缓存，可以是小数，最低 2GB") }}
      </n-text>
    </div>
    <n-input-group class="set">
      ...
      <n-select
        ...
        :options="[
          { label: trSetting('不限制'), value: 0 },
          { label: cacheLimited === 0 ? trSetting('自定义大小 (GB)') : 'GB', value: 1 },
        ]"
        :style="{

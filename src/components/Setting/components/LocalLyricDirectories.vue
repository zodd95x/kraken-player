<template>
  <n-card class="set-item" content-style="flex-direction: column; padding: 16px;">
    <n-flex justify="space-between" align="center" style="width: 100%">
      <div class="label">
        <n-text class="name">{{
          trSetting(item?.label) || trSetting("本地歌词覆盖在线歌词")
        }}</n-text>
        <n-text
          class="tip"
          :depth="3"
          v-if="item?.description"
          v-html="trSetting(item.description)"
        />
        <n-text class="tip" :depth="3" v-else>
          {{ trSetting("可在这些文件夹及其子文件夹内覆盖在线歌曲的歌词") }} <br />
          {{ trSetting("将歌词文件命名为 `歌曲ID.后缀名` 或者 `任意前缀.歌曲ID.后缀名` 即可") }}
          <br />
          {{ trSetting("支持 .lrc 和 .ttml 格式") }} <br />
          {{ trSetting("（提示：可以在前缀加上歌名等信息，也可以利用子文件夹分类管理）") }}
        </n-text>
      </div>

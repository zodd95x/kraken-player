    ...
    <n-card v-else class="set-item">
      <div class="label">
        ...
        <n-text class="tip" :depth="3" v-if="descriptionContent">
          <span
            v-if="typeof descriptionContent === 'string'"
            v-html="trSetting(descriptionContent)"
          />
      ...
      <div class="control-wrapper">
        ...
        <n-input-number
          ...
          v-bind="item.componentProps"
        >
          <template #prefix v-if="item.prefix">{{ trSetting(resolve(item.prefix)) }}</template>
          <template #suffix v-if="item.suffix">{{ trSetting(resolve(item.suffix)) }}</template>
        </n-input-number>
        ...
        <n-input
          ...
          v-model:value="modelValue"
          class="set"
          :placeholder="trSetting(item.componentProps?.placeholder)"
        ...
        <n-button
          ...
          v-bind="item.componentProps"
        >
          {{ trSetting(resolve(item.buttonLabel)) || trSetting("配置") }}
        </n-button>
...
const title = computed(() => {
  if (isForcedConditionMet.value) {
    ...
    if (forcedTitleRef !== undefined) return trSetting(toValue(forcedTitleRef));
  }
  return trSetting(toValue(props.item.title));
});
...
const normalizedOptions = computed(() => {
  ...
  const opts = toValue(props.item.options);
...
const renderSelectLabel = (option: any) => {
  if (option.icon) {
    return h("div", { style: "display: inline-flex; align-items: center; gap: 8px;" }, [
      h(SvgIcon, { name: option.icon, size: 16 }),
      h("span", typeof option.label === "string" ? trSetting(option.label) : option.label),
    ]);
  }
  return typeof option.label === "string" ? trSetting(option.label) : option.label;
};

// 自定义已选标签渲染（支持 SVG 图标）
const renderSelectTag = ({ option }: { option: any }) => {
  if (option.icon) {
    return h("div", { style: "display: inline-flex; align-items: center; gap: 8px;" }, [
      h(SvgIcon, { name: option.icon, size: 16 }),
      h("span", typeof option.label === "string" ? trSetting(option.label) : option.label),

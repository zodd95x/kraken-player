...
export const useGeneralSettings = (): SettingConfig => {
  ...
  const handleModeChange = (val: boolean) => {
    if (val) {
      window.$dialog.warning({
        title: trSetting("开启在线服务"),
        content: trSetting("确定开启软件的在线服务？更改将在热重载后生效！"),
        positiveText: trSetting("开启"),
      ...
      window.$dialog.warning({
        title: trSetting("关闭在线服务"),
        content: trSetting(
          "确定关闭软件的在线服务？关闭后将只能播放本地音乐！更改将在热重载后生效！",
        ),
        positiveText: trSetting("关闭"),
        negativeText: trSetting("取消"),
        onPositiveClick: async () => {
  ...
  const exportSettings = async () => {
    try {
      ...
      if (result && result.success) {
        window.$message.success(`${trSetting("设置导出成功: ")}${result.path}`);
      } else {
        const errorMsg =
          result?.error === "cancelled" ? trSetting("已取消导出") : trSetting("设置导出失败");
        if (result?.error !== "cancelled") {
          window.$message.error(errorMsg);
        }
      }
    } catch {
      window.$message.error(trSetting("设置导出出错"));
    }
  };

  const importSettings = async () => {
    window.$dialog.warning({
      title: trSetting("导入设置"),
      content: () =>
        h("div", null, [
          h(
            NAlert,
            ...
            {
              default: () =>
                trSetting("导入设置将覆盖当前所有配置（包括主题、快捷键、音效设置等）并重启软件。"),
            },
          ),
          h("div", null, trSetting("是否继续？")),
        ]),
      positiveText: trSetting("确定"),
      negativeText: trSetting("取消"),
      onPositiveClick: async () => {
        try {
          ...
          if (result && result.success) {
            ...
            let restoredCount = 0;
            if (data.renderer) {
              const storesToRestore = ["setting-store", "shortcut-store"];
            ...
            if (restoredCount > 0 || data.electron) {
              window.$message.success(trSetting("设置导入成功，即将重启"));
              setTimeout(() => {
                window.location.reload();
              }, 1000);
            } else {
              window.$message.warning(trSetting("未找到可恢复的设置数据"));
            }
          } else {
            if (result?.error !== "cancelled") {
              window.$message.error(
                `${trSetting("设置导入失败: ")}${result?.error || trSetting("未知错误")}`,
              );
            }
          }
        } catch (error) {
          window.$message.error(trSetting("设置导入出错"));
          console.error(error);
        }
      },
    });
  ...
  const resetSetting = () => {
    window.$dialog.warning({
      title: trSetting("警告"),
      content: trSetting("此操作将重置所有设置，是否继续?"),
      positiveText: trSetting("确定"),
      negativeText: trSetting("取消"),
      onPositiveClick: () => {
        settingStore.$reset();
        if (isElectron) window.electron.ipcRenderer.send("reset-setting");
        window.$message.success(trSetting("设置重置完成"));
      },
    });
  };
  ...
  const clearAllData = () => {
    window.$dialog.warning({
      title: trSetting("高危操作"),
      content: trSetting("此操作将重置所有设置并清除全部数据，同时将退出登录状态，是否继续?"),
      positiveText: trSetting("确定"),
      negativeText: trSetting("取消"),
      onPositiveClick: async () => {
        ...
        if (isElectron) window.electron.ipcRenderer.send("reset-setting");
        window.$message.loading(trSetting("数据清除完成，软件即将热重载"), {
          duration: 3000,
  ...
  return {
    groups: [
        ...
        items: [
          {
            ...
            label: "界面语言",
            type: "select",
            description: "选择界面显示语言（法语、英语、简体中文）",
            options: [
              { label: "English", value: "en" },
              { label: "Français", value: "fr" },
              { label: "简体中文 (Original)", value: "zh" },
            ],
            value: computed({
              get: () => settingStore.language,
              set: (v: "fr" | "en" | "zh") => {
                settingStore.language = v;
              },
            }),
          },
        ],
      },
      {
        ...
        items: [
          {
            key: "homeContentStyle",
            label: "首页推荐内容风格",
            type: "select",
            description: "选择首页展示的歌单和音乐推荐风格",
            options: [
              { label: "全球与欧美热歌 (Spotify 风格)", value: "western" },
              { label: "所有分类 (包含中文推荐)", value: "all" },
            ],
            value: computed({
              get: () => settingStore.homeContentStyle,
              set: (v: "western" | "all") => {
                settingStore.homeContentStyle = v;
              },
            }),
          },
        ],

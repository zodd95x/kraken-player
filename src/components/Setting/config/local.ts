...
export const useLocalSettings = (): SettingConfig => {
  ...
  const openDownloadFolder = () => {
    const p = settingStore.downloadPath;
    if (p) {
      window.electron.ipcRenderer.send("open-folder", p);
    } else {
      window.electron.ipcRenderer.invoke("get-default-download-dir").then((defaultPath: string) => {
        if (defaultPath) {
          settingStore.downloadPath = defaultPath;
          window.electron.ipcRenderer.send("open-folder", defaultPath);
        }
      });
    }
  };
        ...
        items: [
          {
            ...
            description: computed(
              () => settingStore.downloadPath || "默认存放在 系统音乐目录/Kraken Player",
            ),
            buttonLabel: "更改",
          ...
          },
          {
            ...
            label: "一键直接下载",
            type: "switch",
            description: "点击下载按钮直接下载到默认目录，无需弹出音质选择窗口",
            value: computed({
              get: () => settingStore.directDownload,
              set: (v) => (settingStore.directDownload = v),
            }),
          },
          {
          ...
          },
          {
            ...
            description: () =>
              h("div", {
                innerHTML:
                  "下载的内嵌歌词将转换为繁体中文<br />使用歌词设置中的繁体变体：" +

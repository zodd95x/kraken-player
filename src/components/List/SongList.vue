...
<template>
  <Transition name="fade">
      ...
      <Transition name="fade">
        ...
        >
          ...
          <div class="list-header song-card sticky-header">
            ...
            >
              <template #trigger>
                <div class="title has-sort">
                  <n-text>{{ trSetting("标题") }}</n-text>
                  <n-text v-if="statusStore.listSortField !== 'default'" class="sort" depth="3">
                    {{ trSetting(sortFieldOptions[statusStore.listSortField].name) }}
                  </n-text>
                </div>
              </template>
              <div class="sort-menu">
                <div class="group">
                  <div class="label">{{ trSetting("排序字段") }}</div>
                  <n-radio-group
                ...
                <div class="group">
                  <div class="label">{{ trSetting("排序方式") }}</div>
                  <n-radio-group
            ...
            <n-text v-else class="title">{{ trSetting("标题") }}</n-text>
            <n-text
              v-if="
            ...
            <span v-if="type !== 'radio' && settingStore.showSongOperations" class="actions" />
            <n-text v-if="type === 'radio' && !isSmallScreen" class="meta date">
              {{ trSetting("更新日期") }}
            </n-text>
            <n-text v-if="type === 'radio' && !isSmallScreen" class="meta">
              {{ trSetting("播放量") }}
            </n-text>
            ...
            <template #default="{ item, index }">
              <div v-if="item.type === 'song'" class="song-node">
                ...
                <SongCard
                  ...
                  @mousedown="
                    draggable
                      ? handlePointerDown($event, index, item.data.name || trSetting('未知曲目'))
                      : undefined
                  "
                  @touchstart="
                    draggable
              ...
              <div v-else-if="item.type === 'footer'" class="load-more">
                <n-flex v-if="loadMore && loading">
                  <n-spin size="small" />
                  <n-text>{{ trSetting(loadingText || "努力加载中...") }}</n-text>
                </n-flex>
                <n-divider v-else dashed> {{ trSetting("没有更多啦 ~") }} </n-divider>
              </div>
            </template>
          </VirtualScroll>
        </div>
    ...
    <n-empty
      v-else
      :description="t('common.empty') || 'Aucun morceau disponible'"
      size="large"

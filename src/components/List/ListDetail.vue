      ...
      <div v-if="detailData" class="detail">
        ...
        <div class="data">
          ...
          <n-collapse-transition :show="!listScrolling" class="collapse">
            <!-- 简介 -->
            <n-text
              v-if="displayDescription && settingStore.playlistPageElements.description"
            ...
            <n-flex class="meta">
              ...
              >
                ...
                >
                  <n-text class="ar" :class="{ 'no-jump': !detailData.artists }">
                    {{
                      settingStore.hideBracketedContent
                        ? removeBrackets(
                            typeof detailData.artists === "string" ? detailData.artists : undefined,
                          )
                        : detailData.artists || trSetting("未知歌手")
                    }}
                  </n-text>
                </div>
                <n-text v-else-if="config.showCreator">
                  {{ localizeCreatorName(detailData.creator?.name) || trSetting("未知用户") }}
                </n-text>
              </div>
              <!-- 歌曲数量 -->
              ...
              <!-- 标签 -->
              <div
                v-if="displayTags.length && settingStore.playlistPageElements.tags"
                class="item hidden"
              >
...
const titleText = computed(() => {
  ...
  return localizePlaylistTitle(props.detailData?.name || "") || trSetting("未知");
});

// 描述文本，非中文模式下隐藏中文简介
const displayDescription = computed(() => {
  const desc = props.detailData?.description || "";
  if (!desc) return "";
...
const displayTags = computed<string[]>(() => {
  ...
  if (settingStore.language === "zh") return tags;
...
const handleDescriptionClick = () => {
  if (displayDescription.value) {
    openDescModal(displayDescription.value, props.titleText || trSetting("查看详情"));
  }
};

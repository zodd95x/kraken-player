    ...
    <n-drawer-content :native-scrollbar="false" closable>
      <template #header>
        <div class="playlist-header">
          <n-text class="name">File d'attente</n-text>
          <n-text class="count" depth="3"> {{ dataStore.playList.length }} titre(s) </n-text>
        </div>
      </template>
          ...
          <template #default="{ item: songData, index }">
            <div class="song-node">
              ...
              >
                ...
                <div
                  class="drag-handle"
                  @mousedown="handlePointerDown($event, index, songData.name || 'Titre inconnu')"
                  @touchstart.passive="
                    handlePointerDown($event, index, songData.name || 'Titre inconnu')
                  "
                  @click.stop
                >
                ...
                <!-- 信息 -->
                <div class="data">
                  <n-text class="name text-hidden">{{ songData.name || "Titre inconnu" }}</n-text>
                    ...
                    <n-text class="ar" depth="3">
                      {{
                        settingStore.hideBracketedContent
                          ...
                          : songData?.artists || "Artiste inconnu"
                      }}
                    </n-text>
                  </div>
                </div>

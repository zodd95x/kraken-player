  ...
  <div class="song-card">
    <div :class="['song-content', { play: musicStore.playSong.id === song.id }]">
      ...
      <div class="num" @dblclick.stop>
        ...
        />
        <!-- 播放 -->
        <SvgIcon :size="28" name="Play" class="play" @click.stop="emit('play', song)" />
      </div>
      <!-- 标题 -->
      <div class="title">
        <!-- 封面 -->
        <s-image
          v-if="!hiddenCover"
          :key="song.cover"
          :src="song.path ? song.cover || song.coverSize?.s : song.coverSize?.s || song.cover"
          class="cover"
        />
        <!-- 信息 -->
          ...
          <n-flex :size="4" :wrap="false" class="desc" align="center">
            ...
            <template v-if="settingStore.showSongArtist">
              ...
              <div v-else-if="song.type === 'radio'" class="artists">
                <n-text class="ar"> {{ trSetting("电台节目") }} </n-text>
              </div>
              <div v-else class="artists" @click="openJumpArtist(song.artists)">
                <n-text class="ar" :class="{ 'no-jump': !song.artists }">
                  {{
                    song.artists
                      ? trSetting(
                          settingStore.hideBracketedContent
...
const albumName = computed(() => {
  ...
  const raw = (settingStore.hideBracketedContent ? removeBrackets(name) : name) || "未知专辑";
  return trSetting(raw);
});
</script>

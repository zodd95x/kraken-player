  ...
  >
          ...
          <n-text class="name text-hidden">
            {{
              dataStore.userLoginStatus
                ? dataStore.userData.name || trSetting("未知用户名")
                : trSetting("未登录")
            }}
          </n-text>
          <!-- VIP -->
    ...
    <div class="user-menu" @click="userMenuShow = false">
      ...
      <n-flex v-else align="center" vertical>
        <n-text>{{ trSetting("UID 登录模式") }}</n-text>
        <n-text :depth="3">{{ trSetting("部分功能暂不可用") }}</n-text>
      </n-flex>
      <n-divider />
      <!-- 多账号 -->
      <div class="account-list" v-if="dataStore.userLoginStatus && dataStore.loginType !== 'uid'">
        <n-text class="subtitle" :depth="3">{{ trSetting("切换账号") }}</n-text>
        <div

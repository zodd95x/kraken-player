  ...
  <n-flex :size="8" align="center" class="right-menu">
    ...
    <template v-if="settingStore.showPlayerQuality">
      ...
      >
        <template #header>
          <n-flex class="quality-title" size="small" vertical>
            <span class="title">{{ trSetting("音质选择") }}</span>
            <span class="tip">{{ trSetting("根据歌曲授权显示") }}</span>
          </n-flex>
        </template>
      ...
      <n-popover v-else trigger="hover" placement="top" :show-arrow="false">
        ...
        <span>{{ trSetting("当前歌曲不支持切换音质") }}</span>
      </n-popover>
    </template>
    <!-- 桌面歌词 -->
    <n-badge
...
const controlsOptions = computed<DropdownOption[]>(() => [
  ...
  {
    label: trSetting("定时关闭"),
    key: "autoClose",
    icon: renderIcon("TimeAuto"),
  },
  {
    label: trSetting("A-B 循环"),
    key: "abLoop",
    icon: renderIcon("Repeat"),
  },
  {
    label: trSetting("倍速播放"),
    key: "rate",
  ...
  switch (key) {
    case "equalizer":
      if (!audioManager.capabilities.supportsEqualizer) {
        window.$message.warning(trSetting("当前音频引擎不支持均衡器"));
        return;
      }
      openEqualizer();
      break;

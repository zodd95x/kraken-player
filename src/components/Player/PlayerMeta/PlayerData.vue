      ...
      <span class="name-text text-hidden">
        {{
          settingStore.hideBracketedContent
            ...
            : musicStore.playSong.name || trSetting("未知歌曲")
        }}
      </span>
      <!-- 额外信息 -->
      <n-flex
          ...
          <div class="player-tip">
            {{
              musicStore.playSong.pc
                ? trSetting("云盘歌曲，由用户上传")
                : trSetting("该歌曲暂时无法播放，为您采用其他音源，可能会与原曲存在差别")
            }}
          </div>
        </n-popover>
      </n-flex>
    ...
    <n-flex :align="center ? 'center' : undefined" size="small" vertical>
      ...
      >
        ...
        <span v-if="settingStore.showPlayerQuality" class="meta-item">
          {{ !statusStore.songQuality ? trSetting("未知音质") : statusStore.songQuality }}
        </span>
        <!-- 歌词模式 -->
        <n-popselect
        ...
        <div v-else class="ar-list">
          <span class="ar">{{
            settingStore.hideBracketedContent
              ...
              : musicStore.playSong.artists || trSetting("未知艺术家")
          }}</span>
        </div>
      </div>
        ...
        >
          {{
            (settingStore.hideBracketedContent
              ...
              : musicStore.playSong.album?.name) || trSetting("未知专辑")
          }}
        </span>
        <span v-else class="name-text text-hidden">
          {{
            (settingStore.hideBracketedContent
              ...
              : musicStore.playSong.album) || trSetting("未知专辑")
          }}
        </span>
      </div>
      <!-- 电台 -->
      <div v-if="musicStore.playSong.type === 'radio'" class="dj" @click="jumpToRadio">
        ...
        <span class="name-text text-hidden">{{
          musicStore.playSong.dj?.name || trSetting("播客电台")
        }}</span>
      </div>
    </n-flex>
  </div>
...
const lyricSourceOptions = computed(() => {
  const options = [
    { label: trSetting("自动"), value: "auto" },
    { label: trSetting("官方优先"), value: "official" },
  ];
  if (settingStore.enableQQMusicLyric) {
    options.push({ label: trSetting("QM 优先"), value: "qm" });
  }
  if (settingStore.enableOnlineTTMLLyric) {
    options.push({ label: trSetting("TTML 优先"), value: "ttml" });
  }
  return options;
});
...
const audioSourceOptions = computed(() => {
  const options = [{ label: trSetting("自动"), value: "auto" }];
...
const audioSourceText = computed(() => {
  if (musicStore.playSong.path) return trSetting("本地");
  if (musicStore.playSong.type === "streaming") return trSetting("流媒体");
  if (musicStore.playSong.pc) return trSetting("云盘");

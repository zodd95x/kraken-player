 ...
 * @param lyricData 歌词数据
 */
export const isChineseLyrics = (lyricData: SongLyric): boolean => {
  if (!lyricData) return false;
  const lines =
    lyricData.lrcData && lyricData.lrcData.length > 0 ? lyricData.lrcData : lyricData.yrcData || [];
  ...
  for (const line of lines) {
    ...
    // 剔除括号内容后，只保留原歌词主干文本进行语言特征检测
    const stripped = text.replace(/[[（(【{「『][^\]）)】}」』]*[\]）)】}」』]/g, "");
    const nonSpace = (stripped.match(/\S/g) || []).length;
    const chinese = (stripped.match(/[\u4e00-\u9fa5]/g) || []).length;
    totalChars += nonSpace;
    chineseChars += chinese;
  }

  return totalChars > 0 && chineseChars / totalChars >= 0.25;
};

/**
 * 过滤非中文歌曲中的中文翻译与嵌在歌词中的中文括号内容
 * @param lyricData 歌词数据
 */
export const filterChineseFromNonChineseLyrics = (lyricData: SongLyric): SongLyric => {
  if (!lyricData) return lyricData;

  // 中文歌曲保留原内容
  if (isChineseLyrics(lyricData)) {
    return lyricData;
  }

  const newLyricData = cloneDeep(lyricData);
  const bracketChineseRegex =
    /[[（(【{「『][^\]）)】}」』]*[\u4e00-\u9fa5]+[^\]）)】}」』]*[\]）)】}」』]/g;
  ...
  const processLine = (line: LyricLine) => {
    ...
    const fullLine = line.words.map((w) => w.word).join("");
    if (!/[\u4e00-\u9fa5]/.test(fullLine)) {
      return;
    }

    // 单词模式
    if (line.words.length === 1) {
      let text = line.words[0].word;
      text = text.replace(bracketChineseRegex, "");
    ...
    line.words = line.words.map((w) => {
      const word = w.word.replace(bracketChineseRegex, "");
      return { ...w, word };
    });

    // 处理跨词包含中文括号的情况
    const remainingFull = line.words.map((w) => w.word).join("");
    if (/[\u4e00-\u9fa5]/.test(remainingFull)) {
      const hasChinese = line.words.map((w) => /[\u4e00-\u9fa5]/.test(w.word));
      ...
      for (let i = 0; i < line.words.length; i++) {
        if (hasChinese[i]) {
          toRemove.add(i);
          for (let b = i - 1; b >= 0; b--) {
            if (/^[[（(【{「『\s]+$/.test(line.words[b].word)) {
              toRemove.add(b);
              break;
            }
            if (line.words[b].word.trim()) break;
          }
          for (let f = i + 1; f < line.words.length; f++) {
            if (/^[\]）)】}」』\s]+$/.test(line.words[f].word)) {
              toRemove.add(f);
              break;
            }
            if (line.words[f].word.trim()) break;
          }
        }
      }

      line.words = line.words.filter((_, idx) => !toRemove.has(idx));
    }
    ...
    line.words = line.words
      .map((w) => ({
        ...w,
        word: w.word.replace(chineseCharRegex, ""),
      }))
      .filter((w) => w.word.trim().length > 0);

    if (line.words.length === 0) {
      line.words = [{ word: "", startTime: line.startTime || 0, endTime: line.endTime || 0 }];
    }
  };

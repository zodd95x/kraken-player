      ...
      if (addedCount > 0 && skippedCount > 0) {
        if (settingStore.language === "en") {
          window.$message.success(
            `Successfully added ${addedCount} folder(s), skipped ${skippedCount} duplicate(s)`,
          );
        } else if (settingStore.language === "fr") {
          window.$message.success(
            `${addedCount} dossier(s) ajouté(s), ${skippedCount} doublon(s) ignoré(s)`,
          );
        } else {
        ...
        if (settingStore.language === "en") {
          window.$message.success(`Successfully added ${addedCount} folder(s)`);
        } else if (settingStore.language === "fr") {
          window.$message.success(`${addedCount} dossier(s) ajouté(s)`);
        } else {
          window.$message.success(`成功添加 ${addedCount} 个目录`);
        }
      } else if (skippedCount > 0) {
        if (settingStore.language === "en") {
          window.$message.warning("Selected folder(s) already exist or overlap, skipped");
        } else if (settingStore.language === "fr") {
...
export const handleSongQuality = (
  ...
  // Anti-AI 过滤：如果是 AI 音质，跳过 level 属性判断，让后续遍历逻辑来确定真正的最高音质
  const isAiLevel =
    disableAiAudio &&
    typeof song === "object" &&
    song &&

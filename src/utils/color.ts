...
const getThemeSchema = (theme: Theme, variant: keyof Theme["palettes"] = "secondary") => {
  const palette = theme.palettes[variant] || theme.palettes.secondary;
...
export const setGlobalColor = (name: string, colorValue: string): void => {
  ...
  if (typeof document !== "undefined") {
    ...
    if (document.body) {
      document.body.style.setProperty(name, colorValue);
    }
  }
};
 ...
 * 仅在自适应封面开启时使用封面颜色
 */
export const resolveTaskbarThemeColor = (): { light: string; dark: string } | null => {
  const statusStore = useStatusStore();
  const settingStore = useSettingStore();
  if (!settingStore.themeFollowCover) return null;

  const themeData = statusStore.songCoverTheme;

  if (!themeData?.dark?.primary || !themeData?.light?.primary) return null;
  const darkPrimary = themeData.dark.primary;
  const lightPrimary = themeData.light.primary;
  return {
...
export const sendTaskbarCoverColor = () => {
  sendTaskbarThemeColor(resolveTaskbarThemeColor());
};

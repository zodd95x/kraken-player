...
class SongDownloadStrategy implements DownloadStrategy {
  ...
  async prepare(): Promise<void> {
    ...
    if (this.shouldDownloadLyrics()) {
      const lyricResult = (await songLyric(this.song.id)) as LyricResult;
      ...
      // 处理基础歌词
      this.basicLyric = await LyricProcessor.processBasic(lyricResult, options);
    }
  ...
  getDownloadConfig(): DownloadConfig {
    ...
    const { downloadMeta, downloadCover, downloadThreadCount, enableDownloadHttp2 } =
      this.settingStore;
   ...
   */
  async postProcess(_downloadedFilePath: string): Promise<void> {
    // 不再生成外部歌词文件
  }

  private async resolveUrl(): Promise<{ url: string; type: string }> {
    const levelName = songLevelData[this.quality]?.level || "exhigh";
    ...
    try {
      ...
      if (result && result.code === 200 && result?.data?.url) {
        return {
          url: result.data.url,
          type: (result.data.type || "mp3").toLowerCase(),
        };
      }
    } catch (e) {
      console.warn("Official download URL failed, falling back to playback stream:", e);
    }

    // 2. 降级尝试：播放接口链接（高码率流媒体音频）
    try {
      const playResult = await songUrl(this.song.id, levelName as Parameters<typeof songUrl>[1]);
      const songData = Array.isArray(playResult?.data) ? playResult.data[0] : playResult?.data?.[0];
      if (songData?.url) {
        return {
          url: songData.url,
      ...
      console.warn("Playback URL fallback failed, falling back to unlock servers:", e);
    }

    // 3. 降级尝试：多音源解锁服务 (Bodian, Kuwo, Netease Unlock)
    try {
      ...
      console.warn("Unlock fallback failed:", e);
    }

    // 4. 降级尝试：本地音乐缓存检查
    if (isElectron) {
      try {
        const cachedPath = await window.electron.ipcRenderer.invoke(
          "music-cache-check",
          this.song.id,
        );
        if (cachedPath) {
        ...
        console.warn("Cache check fallback failed:", e);
      }
    }

    // 全部来源获取失败，抛出本地化错误
    throw new Error(trSetting("获取下载链接失败"));
  }
  /**
   * 获取文件名
   * @returns 文件名
  ...
  private getFileName(): string {
    const unknownSong = trSetting("未知歌曲");
    const unknownArtist = trSetting("未知歌手");
    const infoObj = getPlayerInfoObj(this.song) || {
      name: this.song.name || unknownSong,
      artist: unknownArtist,
    };
    const baseTitle = infoObj.name || unknownSong;
    const rawArtist = infoObj.artist || unknownArtist;
  ...
  private getDownloadPath(): string {
    const finalPath = this.settingStore.downloadPath || "Music/Kraken Player";
    const unknownArtist = trSetting("未知歌手");
    const unknownAlbum = trSetting("未知专辑");
    const infoObj = getPlayerInfoObj(this.song) || { artist: unknownArtist, album: unknownAlbum };
    const safeArtist = (infoObj.artist || unknownArtist).replace(/[/:*?"<>|]/g, "&");
    const safeAlbum = (infoObj.album || unknownAlbum).replace(/[/:*?"<>|]/g, "&");
...
class DownloadManager {
  ...
  public async getDownloadedSongs(): Promise<Record<string, unknown>[]> {
    ...
    if (!isElectron) return [];
    let downloadPath = settingStore.downloadPath;
    if (!downloadPath) {
      try {
        downloadPath = await window.electron.ipcRenderer.invoke("get-default-download-dir");
        if (downloadPath) {
          settingStore.downloadPath = downloadPath;
        }
      } catch (e) {
  ...
  public async addDownload(song: SongType, quality: SongLevelType) {
    ...
    if (isElectron && !settingStore.downloadPath) {
      try {
        ...
        if (defaultPath) settingStore.downloadPath = defaultPath;
      } catch (err) {
    ...
    try {
      ...
      if (isElectron) {
        ...
        if (downloadResult.status === "success" || downloadResult.status === "skipped") {
          ...
          window.$message.success(`${strategy.name} ${trSetting("下载完成")}`);
          // 触发下载完成全局事件以刷新已下载列表
          window.dispatchEvent(new CustomEvent("download-completed", { detail: strategy.id }));
        } else {
          if (downloadResult.status === "cancelled") {
            // 已取消，无需处理
          } else {
            throw new Error(trSetting(downloadResult.message || "下载失败"));
          }
        }
      } else {
        // 浏览器端兜底处理
      ...
      window.$message.error(trSetting(error.message || "下载出错"));
    } finally {

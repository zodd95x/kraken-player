...
import type { MetadataParam, PlaybackStatus, RepeatMode } from "@emi";
...
/**
 * 转发频谱柱状数据到任务栏窗口
 * @param payload 下采样后的 0-255 柱状值
 */
export const broadcastTaskbarSpectrum = (payload: SpectrumPayload) =>
  sendIpc(TASKBAR_IPC_CHANNELS.SYNC_SPECTRUM, payload);

/**
 * 节流版频谱转发（约 20fps，避免 IPC 过载）
 */
export const sendTaskbarSpectrumThrottled: (payload: SpectrumPayload) => void = throttle(
  (payload: SpectrumPayload) => broadcastTaskbarSpectrum(payload),
  50,
);
...
/**
 * @description 通过外部媒体集成模块更新媒体控件的元数据
...
/**
 * @description 通过外部媒体集成模块更新媒体控件的播放状态
 * @note 仅在 Electron 上有效
...
/**
 * @description 通过外部媒体集成模块更新媒体控件的播放模式
 * @note 仅在 Electron 上有效

...
class PlayerController {
  ...
  /** 无限时长诊断节流戳 */
  private lastInfiniteDurationWarn = 0;
  /** 当前歌曲分析结果 */
  ...
  private async parseLocalMusicInfo(path: string) {
    try {
      ...
      // 更新媒体会话
      mediaSessionManager.updateMetadata();
      // 更新任务栏歌词（任务栏窗口读不到 blob:，转 dataURL 再发）
      let widgetCover = musicStore.playSong.cover || "";
      if (widgetCover.startsWith("blob:")) {
        try {
          const coverBlob = await (await fetch(widgetCover)).blob();
          widgetCover = await new Promise<string>((resolve) => {
  ...
  private bindAudioEvents() {
    ...
    audioManager.addEventListener("play", () => {
      ...
      window.document.title = `${playTitle} | Kraken Player`;
      // 只有真正播放了才重置重试计数
    ...
    audioManager.addEventListener("pause", () => {
      ...
      if (!isElectron) window.document.title = "Kraken Player";
      playerIpc.sendPlayStatus(false);
      playerIpc.sendTaskbarState({ isPlaying: false });
      playerIpc.sendTaskbarMode("paused");
      playerIpc.sendTaskbarProgress(statusStore.progress);
    ...
    this.onTimeUpdate = throttle(() => {
      ...
      // 引擎报无限时长（直播流/分片流）时用歌曲元数据时长回退
      const engineMs = audioManager.duration;
      const metaMs = musicStore.playSong?.duration ?? 0;
      const duration =
        Number.isFinite(engineMs) && engineMs > 0
          ? Math.floor(engineMs * 1000)
          : Number.isFinite(metaMs) && metaMs > 0
            ? Math.floor(metaMs)
            : statusStore.duration;
      // 诊断：播放中仍为无限时长时节流记录（定位元数据缺失）
      if (!Number.isFinite(duration)) {
        const now = Date.now();
        if (now - this.lastInfiniteDurationWarn > 10000) {
          this.lastInfiniteDurationWarn = now;
          try {
            window.logger?.info(
              `[duration-pipe] infinite engine=${String(engineMs)} meta=${String(metaMs)} store=${String(statusStore.duration)} type=${musicStore.playSong?.type} id=${musicStore.playSong?.id}`,
            );
          } catch {
            // 忽略探针异常
          }
  ...
  /** 获取总时长 (ms)，引擎无限时回退歌曲元数据 */
  public getDuration(): number {
    const statusStore = useStatusStore();
    const musicStore = useMusicStore();
    const audioManager = useAudioManager();
    const duration = audioManager.duration;
    if (Number.isFinite(duration) && duration > 0) return Math.floor(duration * 1000);
    const meta = musicStore.playSong?.duration ?? 0;
  ...
  public shouldSkipSong(song: SongType): boolean {
    ...
    // 跳过电台播客
    if (song.type === "radio") return true;
    // 是否包含 DJ 关键词
  ...
  public async cleanPlayList() {
    ...
    playerIpc.sendTaskbarProgress("none");
    // 通知任务栏清空（空标题 = 隐藏窗口，下首歌自动恢复）
    playerIpc.sendTaskbarState({ isPlaying: false });
    playerIpc.sendTaskbarMetadata({ title: "", artist: "", cover: "" });
  }
  ...
  public async addNextSong(song: SongType, play: boolean = false) {
    ...
    // 若原列表全为电台播客且将播放普通音乐则重置列表
    if (
      play &&
      song.type !== "radio" &&
      dataStore.playList.length > 0 &&
      dataStore.playList.every((s) => s.type === "radio")
    ) {
      await this.updatePlayList([song], song, undefined, { showTip: true, play: true });
      return;
    }
  ...
  public setTaskbarLyricShow(show: boolean) {
    ...
    window.$message.success(`${show ? "已开启" : "已关闭"}任务栏部件`);
  }
...
const PLAYER_CONTROLLER_KEY = "__KRAKEN_PLAYER_CONTROLLER__";

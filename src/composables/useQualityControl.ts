...
export const useQualityControl = () => {
  ...
  const getQualityName = (quality: QualityType | undefined) => {
    ...
    if (song.path) return trSetting("本地");
    if (song.pc) return trSetting("云盘");
    if (song.type === "radio") return trSetting("电台");
    if (statusStore.isUnlocked) return trSetting("已解锁");
    if (!quality) return trSetting("未知");
  ...
  const loadQualities = async (isPreload = false) => {
    ...
    try {
      ...
      if (res.data) {
        ...
        window.$message.warning(trSetting("获取音质信息失败"));
      }
    } catch (error) {
      console.error(`获取音质详情失败${isPreload ? " (预加载)" : ""}:`, error);
      statusStore.availableQualities = [];
      if (!isPreload) {
        window.$message.error(trSetting("获取音质信息失败"));
      }
    }
  };
  // 预加载音质列表
